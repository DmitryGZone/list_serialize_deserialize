...
void List::create_list_from_file(const std::string& inlet_file) {
    ...
    // vector of pairs to write data / rand
    std::vector<std::pair<std::string, int>> parsed_node;
    // string to store values from file
    std::string line;
    ...
    for (size_t i = 0; i < parsed_node.size(); i++) {
        ...
        if (i < parsed_node.size() - 1) nodes[i]->next = nodes[i + 1]; // edges are nullptr
    }
